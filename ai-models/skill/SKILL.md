...
- 综合分 = (编程分×60% + 智能分×40%) × 价格系数；价格系数随每百万 token 有效价格衰减，有效输入价 = 输入价×2.2% + 缓存命中价×97.8%（有真实缓存读价时直接用，否则按模型估算：DeepSeek 约 3%，其他约 10%）。
- 回答模型选型问题时优先用 `-json` 拿全量字段（上下文长度、输入/输出/缓存读/缓存写价格、评分），再结合用户预算/场景给出建议。
- 表格列含：排名、模型 ID、名称、上下文长度、输入/百万、输出/百万、缓存读/百万、缓存写/百万、编程分、智能分、综合得分、是否免费；价格缺失显示 `-`。
